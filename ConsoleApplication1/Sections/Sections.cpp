...
int main()
{
    ...
    /*
        Создает секции
        Секции сами параллелятся на доступные потоки
        1 секцию полностью выполняет 1 поток
        Если поток выполнил секцию и остались свободные - поток берет свободную секцию
    */
